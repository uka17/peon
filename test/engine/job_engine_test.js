...
describe("1 job engine", function () {
  ...
  it("1.1.3 normalizeStepList. stepList should have type Array", (done) => {
    assert.throws(
      jobEngine.normalizeStepList,
      "stepList should have type Array"
    );
    done();
  });
  ...
  it("1.13.1 execute. Step 1 success, quitWithSuccess", async () => {
    let stub = sinon
      .stub(stepEngine, "execute")
      .resolves({ result: true, rowsAffected: 1 });
    try {
  ...
  it("1.13.3 execute. Step 1 success, gotoNextStep, failed to repeat step 2, quitWithFailure finally", async () => {
    ...
    let stub2 = sinon
      .stub(stepEngine, "delayedExecute")
  ...
  it("1.14.2 calculateNextRun. Failed to calculate next run", (done) => {
    let job = JSON.parse(JSON.stringify(testData.jobOK));
    let stub = sinon
      .stub(schedulator, "nextOccurrence")
      .returns({ result: null, error: "dummy" });
  ...
  it("1.15.1 1-minute execution test. Create connection, create 21 jobs, wait 1 minutes, check if records were created in DB", async function () {
    if (config.skipLongTests) {
      ...
      let connection = await connectionEngine.createConnection(
        testData.execution.connection,
        config.testUser
      );
      ...
      for (let index = 0; index < numberOfJobs; index++) {
        ...
        job.steps[0].command = job.steps[0].command.replace(
          "insert_value",
          `Potatoe${index}-${uid}`
        );
