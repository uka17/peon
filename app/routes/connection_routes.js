...
module.exports = function (app, dbclient) {
  app.get(ver + "/connections/count", async (req, res) => {
    ...
    try {
      ...
      if (req.query.filter !== undefined) filter = req.query.filter;
      else filter = "";
  ...
  app.get(ver + "/connections", async (req, res) => {
    ...
    try {
      ...
      let result = await connectionEngine.getConnectionList(
        filter,
        sortingExpression[0],
        sortingExpression[1],
        perPage,
  ...
  app.post(ver + "/connections", async (req, res) => {
    ...
    try {
      ...
      let connectionValidationResult =
      ...
      if (!connectionValidationResult.isValid)
        ...
        let result = await connectionEngine.createConnection(
          connection,
          config.user
        );
  ...
  app.patch(ver + "/connections/:id", async (req, res) => {
    ...
    try {
      ...
      if (!connectionValidationResult.isValid)
        ...
        let result = await connectionEngine.updateConnection(
          req.params.id,
          connection,
          config.user
        );
  ...
  app.delete(ver + "/connections/:id", async (req, res) => {
    ...
    try {
      let result = await connectionEngine.deleteConnection(
        req.params.id,
        config.user
      );
      let resObject = {};
